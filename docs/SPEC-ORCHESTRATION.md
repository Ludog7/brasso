...
> **ADR-05 est amendé par [ADR-14](adr/ADR-14-theme-clair-defaut-couleur-marque.md) (Acceptée, 2026-07-20) sur le seul point du thème par défaut.** La ligne ci-dessus reste en vigueur pour tout le reste — React 18 + Vite, PWA, cible tablette, Tailwind + shadcn/ui, gros boutons. En revanche « **mode sombre par défaut** » est **renversé** : le thème **clair** devient le défaut, le sombre reste disponible comme **choix** (bascule en Options, M10-11). Motif : l'usage réel a montré que l'application sert **aussi** en bureau et en lumière du jour — c'est l'hypothèse d'usage d'ADR-05 qui s'est élargie, pas son raisonnement qui était faux. ADR-14 fixe en outre la **dérivation d'un thème depuis une couleur de marque** (OKLCH, contraste WCAG 2.1, seuils 4,5:1 et 3:1) avec ses valeurs de référence chiffrées, et acte que la préférence de thème est **par poste**, pas par utilisateur.
...
- **UI atelier** : cibles tactiles ≥ 48 px, contraste AA minimum, **thème clair par défaut avec bascule clair/sombre dans les Options** ([ADR-14](adr/ADR-14-theme-clair-defaut-couleur-marque.md), qui renverse le « mode sombre par défaut » d'ADR-05), zéro drag-and-drop sur les parcours Jour J.
...
> **Tranché par Ludo le 2026-07-18** — le brief nomme les profils « admin, brasseur, **trésorier**, RGPD » alors que le code porte `admin, brasseur, **caisse**, rgpd`. Décision : **la clé `caisse` est conservée**. Seul le **libellé affiché** devient « Trésorier / Caisse ». Conséquence : aucune migration, aucune modification de `Role.key`, aucun impact sur la matrice RBAC — le renommage est purement côté présentation (M10-08).
...
| **M10-03** | ADR-14 (amende ADR-05) | Thème **clair par défaut**, **bascule clair/sombre dans les Options**, et thème dérivé d'une couleur de marque. **Sens tranché par Ludo le 2026-07-18** ; le ticket porte la rédaction formelle de l'ADR et la mise en cohérence de la spec. ⚠️ **Renversement d'ADR** : ADR-05 (§0) **et** §6 imposaient « mode sombre par défaut » **jusqu'à l'acceptation d'ADR-14 le 2026-07-20** — **les deux emplacements** devaient être amendés, pas seulement l'ADR ; ce fut fait. Le mode sombre n'est pas supprimé : il devient un choix utilisateur. Contrainte à tenir : **contraste AA dans les deux thèmes**, l'usage cible restant la tablette d'atelier. |
