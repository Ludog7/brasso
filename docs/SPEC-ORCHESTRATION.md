...
| **M10-03** | ADR-14 (amende ADR-05) | Thème **clair par défaut**, **bascule clair/sombre dans les Options**, et thème dérivé d'une couleur de marque. **Sens tranché par Ludo le 2026-07-18** ; le ticket porte la rédaction formelle de l'ADR et la mise en cohérence de la spec. ⚠️ **Renversement d'ADR** : ADR-05 (§0) **et** §6 imposent aujourd'hui « mode sombre par défaut » — **les deux emplacements** doivent être amendés, pas seulement l'ADR. Le mode sombre n'est pas supprimé : il devient un choix utilisateur. Contrainte à tenir : **contraste AA dans les deux thèmes**, l'usage cible restant la tablette d'atelier. |
