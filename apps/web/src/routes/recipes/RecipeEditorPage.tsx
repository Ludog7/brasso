...
export function RecipeEditorPage() {
  ...
  const readOnly = data ? data.status !== "DRAFT" : false;

  // (Ré)initialise le formulaire quand la recette chargée change d'identité ou
  // vient d'être enregistrée (`updatedAt` change → le dirty retombe à zéro).
  useEffect(() => {
    if (!data) {
      return;
    }
  ...
  useBeforeUnload(dirty && !readOnly);
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!data || readOnly || update.isPending) {
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="flex flex-wrap items-center gap-3 border-b border-border px-6 py-4">
        <Button variant="ghost" size="icon" onClick={leave} aria-label="Retour aux recettes">
          <ArrowLeft className="size-5" aria-hidden="true" />
        </Button>
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-lg font-semibold">{data.name}</h1>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
            <span>{ENGINE_LABELS[data.engine]}</span>
      ...
      <main className="mx-auto max-w-2xl p-6">
        {readOnly ? (
          ...
          >
            Cette recette est {STATUS_LABELS[data.status].toLowerCase()} : elle est en lecture
            seule. La création d'une nouvelle version arrivera avec le parcours versions (M2-09).
          </p>
        ) : null}
        ...
        <form className="flex flex-col gap-6" onSubmit={onSubmit} noValidate>
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">
              ...
              jauges) arrivera dans un prochain ticket.
            </CardContent>
          </Card>
