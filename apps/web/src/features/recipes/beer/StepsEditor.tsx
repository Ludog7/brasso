import type { ProcessStepType } from "@brasso/core";
import { stepTypesByEngine } from "@brasso/core";
...
import { emptyStep, type StepRow } from "./mapToEngine";
import { RowField } from "./RowField";

const BEER_STEP_TYPES = stepTypesByEngine.BEER;
...
/** Quels champs afficher selon le type d'étape (aligné sur `stepParams`). */
function fieldsFor(type: ProcessStepType): { temp: boolean; time: boolean; days: boolean } {
  switch (type) {
    case "MASH":
    case "MASH_STEP":
    case "WHIRLPOOL":
      return { temp: true, time: true, days: false };
    case "SPARGE":
...
export function StepsEditor({ steps, disabled, onChange }: StepsEditorProps) {
    ...
    <Card>
      <CardHeader className="flex-row flex-wrap items-center justify-between gap-3">
        ...
        <Button
          ...
          variant="outline"
          disabled={disabled}
          onClick={() => onChange([...steps, emptyStep("MASH_STEP")])}
        >
          + Étape
        </Button>
      </CardHeader>
      <CardContent className="flex flex-col gap-3">
        {steps.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Aucune étape. Ajoute paliers d'empâtage, ébullition, fermentation…
          </p>
        ) : (
          steps.map((row) => {
